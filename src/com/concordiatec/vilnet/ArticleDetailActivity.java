...
import com.concordiatec.vilnet.adapter.ArticleDetailCommentAdapter;
...
import com.concordiatec.vilnet.service.ArticleDetailService;
...
public class ArticleDetailActivity extends SubPageSherlockActivity implements OnClickListener {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					aService.deleteArticle(detail.getId(), new SimpleVicResponseListener() {
						...
						public void onSuccess(ResData data) {
							...
							//리스트에 삭제 알림
							if( detail != null ){
								BroadHelper.sendSerializableBroad( ArticleDetailActivity.this , BroadKeys.ARTICLE_DELETE, detail);
							}
							finish();
						}
					});
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case Constant.COMMENT_EDIT: // 댓글 수정 성공시
			if (resultCode == Constant.COMMENT_EDIT_SUCCED && data.hasExtra("edit_comment")) {
				...
				BroadHelper.sendSerializableBroad( ArticleDetailActivity.this , BroadKeys.COMMENT_CHANGE, editComment);
			}
			break;
		case Constant.ARTICLE_EDIT: // 원글내용 수정 성공시
			if (resultCode == RESULT_OK && data.hasExtra("edit_article")) {
				...
				if( editDetail != null ){
					//리스트에 수정 알림
					BroadHelper.sendSerializableBroad( ArticleDetailActivity.this , BroadKeys.ARTICLE_CHANGE, editDetail);
				}
			}
			break;
		default:
		...
		detailService.getDetail(articleId, new SimpleVicResponseListener() {
			...
			public void onSuccess(ResData data) {
				...
				if (imgs.size() > 1) {
					...
					contentImgLayout.setOnClickListener(ArticleDetailActivity.this);
					...
					contentImg.setOnClickListener(ArticleDetailActivity.this);
				}				
				commentCount.setOnClickListener(ArticleDetailActivity.this);
	...
	private void likeArticle(View v, int articleId) {
		...
		final int likeCount = Integer.parseInt(t.getText().toString());
		...
		case CONTEXT_COMMENT_PLUS_CANCEL:
			...
			commentService.likeComment(clickedComment.getId(), loginUser.usrId, new SimpleVicResponseListener() {
				...
				public void onSuccess(ResData data) {
					...
					BroadHelper.sendSerializableBroad( ArticleDetailActivity.this , BroadKeys.COMMENT_CHANGE, clickedComment);
				}
			});
			break;
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					commentService.deleteComment(clickedComment.getId(), loginUser.usrId, new SimpleVicResponseListener() {
						...
						public void onSuccess(ResData data) {
							...
							NotifyUtil.toast(ArticleDetailActivity.this, getString(R.string.delete_succed));
							BroadHelper.sendSerializableBroad( ArticleDetailActivity.this , BroadKeys.COMMENT_DELETE, clickedComment);
							ProgressUtil.dismiss();
						}
	...
	public void onClick(View v) {
		switch (v.getId()) {
		//이미지 상세보기
		case R.id.content_img_layout:
			if (!imgList.isEmpty()) {
				Intent intent = new Intent(this, ArticleImagesActivity.class);
